...
(function () {
  ...
  tabCloseCenter.addEventListener("click", function () {
    centerTerminal.style.animation = "terminal-out 0.2s ease forwards";
    centerTerminal.addEventListener("animationend", function handler() {
      centerTerminal.classList.add("hidden");
      centerTerminal.style.animation = "";
      centerTerminal.removeEventListener("animationend", handler);
    });
  });

  function showCenterTerminal() {
    if (centerTerminal.style.transform !== "none") {
      centerTerminal.style.left = "";
      centerTerminal.style.top = "";
      centerTerminal.style.transform = "";
    }
    centerTerminal.classList.remove("hidden", "entering");
    void centerTerminal.offsetWidth; // force reflow so animation replays
    centerTerminal.classList.add("entering");
  }
  ...
  // — Click-to-enter overlay + audio unlock —
